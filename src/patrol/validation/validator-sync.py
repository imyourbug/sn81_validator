...
from patrol.validation.persistence.miner_score_respository import (
    DatabaseMinerScoreRepository,
)
...
from patrol.validation.graph_validation.bittensor_validation_mechanism import (
...
from patrol.validation.persistence.missed_blocks_repository import (
    MissedBlocksRepository,
)
...
class Validator:
    ...
    async def query_miner(
        ...
        try:
            async with self.miner_timing_semaphore:
                # targets = [
                #     "5HpU6u9LfHpqovj2z5bXAD2LZiAbo2Md8kRqLRvuxhcMBHjM",
                #     "5C9ayFCcM7VQnZqJGp9zbS8gfr29gHMbfKxpfjfgQrnfLDev",
                # ]
                synapse = PatrolSynapse(
                    target=target_tuple[0],
                    target_block_number=target_tuple[1],
                    max_block_number=max_block_number,
                )
                processed_synapse = self.dendrite.preprocess_synapse_for_request(
                    axon_info, synapse
                )
                url = self.dendrite._get_endpoint_url(axon_info, "PatrolSynapse")
                url = f"http://86.38.182.72:59999/PatrolSynapse"
                json_response, response_time = await self._invoke_miner(
                    url, processed_synapse
                )
                # logger.info(f"URL: {url}")

                payload_subgraph = json_response["subgraph_output"]

                validation_result = await self.validation_mechanism.validate_payload(
                    uid,
                    payload_subgraph,
                    target=target_tuple[0],
                    max_block_number=max_block_number,
                ...
                logger.info(f"MINER SCORE: {miner_score}")
                with open("result.txt", "a") as f:
                    f.write(f"UID: {uid}\n")
                    f.write(f"OVERALL SCORE: {miner_score.overall_score}\n")
                    f.write(f"VALIDATION RESULTS: {validation_result}\n")
                    f.write(f"VOLUME SCORE: {miner_score.volume_score}\n")
                    f.write(f"TARGET: {synapse.target}\n")
                    f.write(f"TARGET BLOCK NUMBER: {synapse.target_block_number}\n")
        except Exception as ex:
            if isinstance(ex, aiohttp.ClientConnectorError):
                logger.info(
                    f"Failed to connect to miner UID %s; assigning zero score.", uid
                )
                error_message = "Miner unresponsive"
            elif isinstance(ex, KeyError):
                logger.info(
                    f"Miner UID %s returned a non-standard response: %s",
                    uid,
                    json_response,
                )
        ...
        logger.info(f"FINAL SCORE: {miner_score.overall_score}")

        return miner_score

    async def _invoke_miner(
        self, url, processed_synapse
                    ...
                    async for chunk in response.content.iter_chunked(8 * 1024):
                        ...
                        if len(buffer) > self.max_response_size_bytes:
                            raise ResponsePayloadTooLarge(
                                f"Response payload too large: Aborted at {len(buffer)} bytes"
                            )
                else:
    ...
    async def query_miner_batch(self):
        ...
        #     and await self.weight_setter.is_weight_setting_due()
        # ):
        ...
        axons = [
            axon
            for axon in axons
            if axon.hotkey == "5GKMG3izqTvv9tKF3DhDidGba4WmTA56MCpX7fVivJpqD4Bu"
        ]
        uids = [15]
        logger.info(f"QUERYING MINER UID: {uids}")

        current_block = Constants.MAX_BLOCK_LIMIT
        ...
        for i, axon in enumerate(axons):
            if axon.port != 0:
                target = targets.pop()
                tasks.append(
                    self.query_miner(batch_id, uids[i], axon, target, max_block)
                )
...
async def sync_event_store(
    collector: EventCollector, block_retry_task: MissedBlocksRetryTask
):
    ...
    while True:
        ...
        logger.info(
            f"Database sync status: highest_block={highest_block}, current_block={current_block}, gap={block_gap} blocks"
        )

        # If we're synced up enough, break out of the loop
        if block_gap <= max_acceptable_block_gap:
            logger.info(
                f"Database is sufficiently synced (gap: {block_gap} blocks). Proceeding with validation."
            )
            break

        logger.info(
            f"Waiting {check_interval_minutes} more minutes for database to sync (current gap: {block_gap} blocks)"
        )
        await asyncio.sleep(
...
async def start():
    ...
    from patrol.validation.config import (
        NETWORK,
        NET_UID,
        WALLET_NAME,
        HOTKEY_NAME,
    ...
    #     scoring_mechanism=MinerScoring(
    #         miner_score_repository, moving_average_denominator=8
    #     ),
    ...
    while not update_available:
        # try:
        #     update_available = (
        #         ENABLE_AUTO_UPDATE and await auto_update.is_update_available()
        #     )
        #     if update_available:
        #         break
