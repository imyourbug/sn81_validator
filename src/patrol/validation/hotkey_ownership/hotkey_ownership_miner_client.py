...
class HotkeyOwnershipMinerClient:
    ...
    async def execute_task(
        self, miner: AxonInfo, synapse: HotkeyOwnershipSynapse
    ) -> tuple[HotkeyOwnershipSynapse, float]:

        processed_synapse = self._dendrite.preprocess_synapse_for_request(
            miner, synapse
        )
        ...
        try:
            async with aiohttp.ClientSession(trace_configs=[trace_config]) as session:
                ...
                response_synapse = HotkeyOwnershipSynapse.model_validate_json(
                    await response.text()
                )
