...
from patrol_mining import Constants
...
class SubgraphGenerator:
    ...
    async def generate_block_numbers(
        self,
        target_block: int,
        upper_block_limit: int,
        lower_block_limit: int = Constants.LOWER_BLOCK_LIMIT,
    ) -> List[int]:

        print(f"Generating block numbers for target block: {target_block}")
    ...
    def generate_adjacency_graph_from_events(self, events: List[Dict]) -> Dict:
        ...
        print(f"Adjacency graph created in {time.time() - start_time} seconds.")
        return graph

    def generate_subgraph_from_adjacency_graph(
        self, adjacency_graph: Dict, target_address: str
    ) -> Dict:
        start_time = time.time()
        ...
        while queue:
            ...
            if current not in seen_nodes:
                nodes.append(Node(id=current, type="wallet", origin="bittensor"))
            ...
            for conn in adjacency_graph.get(current, []):
                ...
                try:
                    ...
                    print(
                        f"Issue with adding edge to subgraph, skipping for now. Error: {e}"
                    )
        ...
        print(
            f"Subgraph graph of length {subgraph_length} created in {time.time() - start_time} seconds."
        )
    ...
    async def run(self, target_address: str, target_block: int, max_block_number: int):
        ...
        block_numbers = await self.generate_block_numbers(
            target_block, upper_block_limit=max_block_number
        )
        ...
        subgraph = self.generate_subgraph_from_adjacency_graph(
            adjacency_graph, target_address
        )
...
if __name__ == "__main__":
    ...
    async def example():
        ...
        client = SubstrateClient(
            runtime_mappings=versions, network_url=network_url, max_retries=3
        )
        ...
        subgraph = await subgraph_generator.run(
            target, target_block, max_block_number=4179351
        )
        ...
        # print(output)
        print(f"Finished: {time.time() - start_time} with volume: {volume}")
