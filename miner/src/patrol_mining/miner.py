...
from patrol_common.protocol import (
    PatrolSynapse,
    HotkeyOwnershipSynapse,
    AlphaSellSynapse,
)
...
class Miner:
    def __init__(
        ...
        min_stake_allowed: int,
        network_url: str,
        max_future_events: int = 50,
        max_past_events: int = 50,
        batch_size: int = 25,
    ):
    ...
    async def blacklist(self, synapse: bt.Synapse) -> tuple[bool, str]:
        ...
        if (
            ...
            or self.metagraph.S[uid] < self.min_stake_allowed
        ):
            return True, "Non-validator hotkey"
        return False, None

    async def blacklist_coldkey_search(
        self, synapse: PatrolSynapse
    ) -> Tuple[bool, str]:
        return await self.blacklist(synapse)

    async def blacklist_hotkey_ownership_search(
        self, synapse: HotkeyOwnershipSynapse
    ) -> Tuple[bool, str]:
        return await self.blacklist(synapse)

    async def blacklist_alpha_sell_prediction(
        self, synapse: AlphaSellSynapse
    ) -> Tuple[bool, str]:
        return await self.blacklist(synapse)

    async def coldkey_search(self, synapse: PatrolSynapse) -> PatrolSynapse:
        print(
            f"Received coldkey search request: {synapse.target}, with block number: {synapse.target_block_number}"
        )
        start_time = time.time()
        future = run_coroutine_threadsafe(
            self.subgraph_generator.run(
                synapse.target, synapse.target_block_number, synapse.max_block_number
            ),
            self.subgraph_loop,
        )
        ...
        print(
            f"Returning a graph of {volume} in {round(time.time() - start_time, 2)} seconds."
        )
        return synapse

    async def hotkey_ownership_search(
        self, synapse: HotkeyOwnershipSynapse
    ) -> HotkeyOwnershipSynapse:
        print(f"Received hotkey ownership request: {synapse.target_hotkey_ss58}")
        start_time = time.time()
        future = run_coroutine_threadsafe(
            self.hotkey_owner_finder.find_owner_ranges(
                synapse.target_hotkey_ss58, max_block=synapse.max_block_number
            ),
            self.subgraph_loop,
        )
        synapse.subgraph_output = future.result()
    ...
    async def process_alpha_sell_prediction(
        self, synapse: AlphaSellSynapse
    ) -> AlphaSellSynapse:
        print(
            f"Received alpha sell request netuid: {synapse.subnet_uid}, interval: {synapse.prediction_interval}"
        )
        start_time = time.time()
        future = run_coroutine_threadsafe(
        ...
        print(
            f"Returning {volume} predictions in {round(time.time() - start_time, 2)} seconds."
        )
        return synapse

    async def setup_axon(self):
        self.axon = bt.axon(
            wallet=self.wallet, port=self.port, external_ip=self.external_ip
        )
        self.axon.attach(
            forward_fn=self.coldkey_search, blacklist_fn=self.blacklist_coldkey_search
        )
        self.axon.attach(
            forward_fn=self.hotkey_ownership_search,
            blacklist_fn=self.blacklist_hotkey_ownership_search,
        )
        self.axon.attach(
            forward_fn=self.process_alpha_sell_prediction,
            blacklist_fn=self.blacklist_alpha_sell_prediction,
        )
    ...
    async def setup_miner(self):
        try:
            ...
            client = SubstrateClient(
                runtime_mappings=versions, network_url=self.network_url, max_retries=3
            )
            ...
            print("Successfully initialised, waiting for requests...")
            return True
        except Exception as e:
            print(f"Unsuccessfuly attempted to set up miner dependencies. Error: {e}")
            exit()

    async def run(self):
        future = run_coroutine_threadsafe(self.setup_miner(), self.subgraph_loop)
        ...
        while True:
            try:
                if step % 60 == 0 and not self.dev_flag:
                    ...
                    print(
                        f"Block: {self.metagraph.block.item()} | Incentive: {self.metagraph.I[self.my_subnet_uid]}"
                    )
                step += 1
                time.sleep(1)
